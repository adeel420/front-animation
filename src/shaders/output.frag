...
float map(vec3 p, vec2[TRAIL_LENGTH] trail, vec2 res) {
    ...
    for (int i = 0; i < TRAIL_LENGTH; i++) {
        float fi = float(i);
        vec2 pointerTrail = trail[i] * res / min(res.x, res.y);
...
vec3 generateNormal(vec3 p, vec2[TRAIL_LENGTH] trail, vec2 res) {
    return normalize(vec3(
        map(p + vec3(EPS, 0.0, 0.0), trail, res) - map(p + vec3(-EPS, 0.0, 0.0), trail, res),
        map(p + vec3(0.0, EPS, 0.0), trail, res) - map(p + vec3(0.0, -EPS, 0.0), trail, res),
        map(p + vec3(0.0, 0.0, EPS), trail, res) - map(p + vec3(0.0, 0.0, -EPS), trail, res)
    ));
}
...
void main() {
    ...
    for (int i = 0; i < ITR; ++i) {
        dist = map(ray, uPointerTrail, uResolution);
    ...
    if (dist < EPS) {
        vec3 normal = generateNormal(ray, uPointerTrail, uResolution);
        color = dropletColor(normal, rayDirection, uTime);
    }
